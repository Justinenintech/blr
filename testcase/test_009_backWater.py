...
from PageObject.page_realTimeBackWater import PageRealTimeBackWater
...
@ddt.ddt
class TestBackWater(unittest.TestCase):
    ...
    assert_back_water = data['asserts'].get('assert_back_water') #实时返水
    ...
    def setUp(self) -> None:
        self.link = PageRealTimeBackWater(self.driver)

    @BeautifulReport.add_test_img('test_back_water_001')
    def test_back_water_001(self):
        """新注册会员，登录情况下，访问实时返水"""
        self.case_name = '新注册会员，登录情况下，访问实时返水'
        ...
        # 点击实时返水
        self.link.click_link_realTime_backwater()
        # 获取实时返水标题
        self.re_data = self.link.verity_get_back_water()
        try:
            # 验证标题是否一致
            self.assertEqual(self.re_data.replace(" ", ""), self.assert_back_water.replace(" ", ""),self.output_verify_fail % (self.assert_back_water.replace(" ", ""), self.re_data.replace(" ", "")))
            # 将结果输出到日志文件
            log1.info(self.output_verify_success % (self.assert_back_water.replace(" ", ""), self.re_data.replace(" ", "")))
            # 保存截图
    ...
    def tearDownClass(cls) -> None:
        cls.link = PageRealTimeBackWater(cls.driver)
        cls.link.dr_quit()
