...
# path = 'D:/vincent/tg-auto-BeautifulReport/'
# d = os.path.join(path, 'img',
#                  'error_img')
# # template_path = os.path.join(os.path.dirname(__file__), 'template')
# # print("123"+ template_path)
# print(d)
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    imgs_path = os.path.join(project_path, 'img')
    # 错误日志存放路径
    error_log_path = os.path.join(imgs_path, 'error_img/')
    # 如果没有错误日志文件夹，创建该文件夹
    if not os.path.exists(error_log_path):
        os.makedirs(error_log_path)
    img_path = error_log_path if platform.system() != 'Windows' else error_log_path
